...
export const Layout = ({ children }: LayoutProps) => {
  const { rol, nombre, id, token, logout } = useContext(AuthContext);
  const navigate = useNavigate();
  const location = useLocation();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  // 🚨 ESTADO PARA CONTROLAR QUÉ SECCIONES ESTÁN OCULTAS 🚨
  const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>({});

  const { isSyncing, syncSuccess, pendingCount } = useSync(id, token);
  ...
  const allowedMenuGroups = menuGroups
    .map(group => ({
      ...group,
      items: group.items.filter(item => rol && item.allowedRoles.includes(rol))
    }))
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-gray-50 bg-gray-50/50 shrink-0">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Sesión Actual</p>
          <div className="flex items-center">
            <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-700 font-bold flex items-center justify-center mr-3 shrink-0">
        ...
        {/* 🚨 MENÚ COLAPSABLE Y ANIMADO 🚨 */}
        <nav className="flex-1 overflow-y-auto py-4 px-3 custom-scrollbar">
          {allowedMenuGroups.map((group, index) => {
            ...
            return (
              <div key={index} className="mb-4">
                ...
                <button 
                  ...
                  className="w-full flex items-center justify-between px-3 py-2 mb-1 group/header hover:bg-gray-100/50 rounded-lg transition-colors focus:outline-none"
                >
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest group-hover/header:text-gray-600 transition-colors">
                    {group.section}
                  </span>
                  <svg 
                    className={`w-3.5 h-3.5 text-gray-400 group-hover/header:text-gray-600 transition-transform duration-300 ease-in-out ${isCollapsed ? '-rotate-90' : 'rotate-0'}`} 
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-50 h-screen overflow-y-auto relative">
        ...
        {/* INDICADOR VISUAL DE SINCRONIZACIÓN GLOBAL */}
